...
from config import (
    SPLITS_DIR, MODELS_DIR, PROCESSED_DIR,
...
class RatingsDataset(Dataset):
    def __init__(self, df: pd.DataFrame,
        ...
        user_feat_cols  = [c for c in user_features.columns
                           if c not in ("user_idx",) and
                           user_features[c].dtype in (np.float64, np.int64, float, int)]
        item_feat_cols  = [c for c in item_features.columns
                           if c not in ("movie_idx", "title") and
                           item_features[c].dtype in (np.float64, np.int64, float, int)]
...
class TwoTowerModel(nn.Module):
    ...
    def __init__(self, n_users: int, n_movies: int,
                 user_feat_dim: int, item_feat_dim: int,
    ...
    def _build_tower(self, input_dim: int,
                     hidden_dims: list, dropout: float) -> nn.Sequential:
        layers = []
        in_dim = input_dim
    ...
    def get_user_vector(self, user_idx: torch.Tensor,
                        user_feat: torch.Tensor) -> torch.Tensor:
        u_emb = self.user_embedding(user_idx)
        return self.user_tower(torch.cat([u_emb, user_feat], dim=1))

    def get_item_vector(self, movie_idx: torch.Tensor,
...
class Trainer:
    def __init__(self, model: TwoTowerModel,
        ...
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            self.optimizer, patience=2, factor=0.5, verbose=True
        )
    ...
    def fit(self, train_loader: DataLoader,
            ...
            n_epochs: int = NUM_EPOCHS) -> list[dict]:
        history = []
        best_rmse = float("inf")
        best_state = None
...
if __name__ == "__main__":
    ...
    with mlflow.start_run(run_name="two-tower-v1"):
        ...
        mlflow.log_metrics({"val_rmse": best["val_rmse"],
