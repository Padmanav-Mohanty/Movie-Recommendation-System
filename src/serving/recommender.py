...
class SVDRecommender(BaseRecommender):
    ...
    def __init__(self, model_path: Path = None, train_df: pd.DataFrame = None):
        ...
        if train_df is not None:
            self._seen = train_df.groupby("user_idx")["movie_idx"].apply(list).to_dict()
        else:
            self._seen = {}
...
class TwoTowerRecommender(BaseRecommender):
    ...
    def _build_item_vectors(self) -> np.ndarray:
        ...
        all_feat = torch.zeros(self._n_movies, self._item_feat_mat.shape[1])
