...
def build_user_features(train: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Basic stats
    user_stats = train.groupby("user_idx")["rating"].agg(
        mean_rating="mean",
        rating_std="std",
        n_ratings="count"
    ).reset_index()
    ...
    genre_affinity.columns = [f"genre_affinity_{g.lower().replace('-','_')}"
                               for g in genre_affinity.columns]
...
def build_item_features(train: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Basic stats
    item_stats = train.groupby("movie_idx")["rating"].agg(
        mean_rating="mean",
        rating_std="std",
        n_ratings="count"
    ).reset_index()
    ...
    movie_meta = (
        train[["movie_idx", "title", "genre_list"]]
...
def build_interaction_matrix(df: pd.DataFrame,
    ...
    print(f"Interaction matrix : {mat.shape} | "
          f"sparsity {1 - mat.nnz / (n_users * n_movies):.4%}")
    return mat


# ── Pipeline ──────────────────────────────────────────────────────────────────

def run_feature_pipeline(train: pd.DataFrame,
                         n_users: int,
                         n_movies: int) -> dict:
